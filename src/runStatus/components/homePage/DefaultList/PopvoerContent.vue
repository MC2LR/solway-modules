...
export default {
    ...
    methods: {
        ...
        change () {
            ...
            const stClass = {'01': '光伏', '02': '风电'}[this.data.stClass]
            const components = ((this.baseSearchConditionGetAllComByRole.res.body && this.data.comIds.length) ? this.baseSearchConditionGetAllComByRole.res.body.filter(v => this.data.comIds.includes(v.comId)).map(v => v.comName).join('、') : '')
            const stations = ((this.baseSearchConditionGetStByRoleAndComIds.res.body && this.data.stIds.length) ? this.baseSearchConditionGetStByRoleAndComIds.res.body.filter(v => this.data.stIds.includes(v.stId)).map(v => v.stName).join('、') : '')
            const deviceTypes = this.data.deviceType.map(v => ({'1': '汇流箱', '2': '逆变器', '18': '风机'}[v])).join('、')
            const devices = (this.data.deviceAll ? '全部设备' : this.custom.sts.reduce((a, b) => {
                return a.concat(this.custom[b].devices.map(v => v.deviceName))
            }, []).join('、') || '全部设备')
            const str = stClass + ' / ' + components + (components && ' / ') + stations + (stations && ' / ') + deviceTypes + ' / ' + devices
