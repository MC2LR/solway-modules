<template>
    <div class="child-section-switch" ref="lowSwitchSection">
        <div class="block-con" :style="{marginRight: marginRight[index] + 'px'}" v-for="(item, index) in arr" :key="index">
            <block
            :title='item.name'
            name='温度'
            :value='!item.tmp && item.tmp != 0 ? "--" : item.tmp == 0 ? 0 : parseFloat(item.tmp.toFixed(2))'
            units='℃'
            icon='icongaoyagui-'
...
export default {
    ...
    computed: {
        bgColor () {
            return function (status) {
                return status === 1 ? 'rgba(93, 182, 108, 1)' : status === 2 ? 'rgba(201, 91, 91, 1)' : status === 3 ? 'rgba(214, 157, 101, 1)' : status === -99 ? 'rgba(179, 179, 179, 1)' : ''
            }
        },
        bgColorOpacity () {
            return function (status) {
                return status === 1 ? 'rgba(93, 182, 108, .2)' : status === 2 ? 'rgba(201, 91, 91, .2)' : status === 3 ? 'rgba(214, 157, 101, .2)' : status === -99 ? 'rgba(179, 179, 179, .2)' : ''
            }
        }
    },
    mounted () {
    ...
    methods: {
        getStatusData () {
            this.getDeviecList.getData({type: 43}).then(res => {
                this.arr = res.data
                this.changeMargin()
            })
        },
        async changeMargin () {
            ...
            this.marginRight = []
            this.$nextTick(() => {
                this.maxNum = parseInt(this.$refs.lowSwitchSection.offsetWidth / 156)
                this.setMargin()
            ...
            this.arr.forEach((v, i) => {
                if (this.arr.length < this.maxNum) {
                    ...
                    if ((i + 1) % this.maxNum === 0 && i !== 0) {
                        ...
                        this.marginRight.push((this.$refs.lowSwitchSection.offsetWidth - this.maxNum * 156) / (this.maxNum - 1))
                    }
                }
            })
        }
