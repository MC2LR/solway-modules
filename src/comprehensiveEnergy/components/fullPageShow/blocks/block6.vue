        ...
        <div class="chargingPile-list">
            <div v-for="item in ceDeviceMonitorRtDeviceList.res.data" :key="item.name" class="item">
                ...
                <div class="chargingPile">
                    <i v-show="item.useStatus === '01'" class="iconfont iconzhuanquan-"></i>
                    <i class="iconfont iconchongdianzhuang2-copy-copy-copy-copy-copy" :class="'s' + item.useStatus"></i>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    created () {
        this.timer = setInterval(() => this.ceDeviceMonitorRtDeviceList.getData(), this.$store.state.second.dataInterval)
    },
    beforeDestroy () {
