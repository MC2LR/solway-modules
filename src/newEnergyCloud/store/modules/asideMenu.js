import router, { resetRouter } from '@newEnergyCloud/router'
import allMenuData from '../allMenu'
import { api＿queryCurrentMenu } from '@newEnergyCloud/request/api'
import Vue from 'vue'
...
const actions = {
    ...
    getMenu ({ commit, dispatch, rootState, state }) {
        return new Promise(async (resolve, reject) => {
            await api＿queryCurrentMenu({ data: { version: 3 } }).subscribe(res => {
                let parseMenuList
                try {
                    parseMenuList = parseMenu(res?.body)
                } catch (error) {
                    parseMenuList = []
