...
export default {
    ...
    mounted () {
        this.$bus.on('updateWatchList', this.getList)
    },
    methods: {
        ...
        getList () {
            this.custDeviceMeterGetCustDeviceMeterList.getData({stId: this.detail.stid})
        },
        // 删除电表
        deleteWatch (row) {
    ...
    beforeDestroy () {
        this.$bus.off('updateWatchList', this.getList)
    }
}
</script>
