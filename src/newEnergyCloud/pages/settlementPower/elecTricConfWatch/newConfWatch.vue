...
export default {
  ...
  mounted () {
    ...
    this.$bus.on('updateElecWatchList', this.getStid)
  ...
  methods: {
    async openWatch () {
      ...
      if (this.detail) {
        ...
        this.custDeviceMeterSaveDeviceMeter.params.stId = this.stid
      } else {
    ...
    submit () {
        this.$refs.Form.validate(valid => {
          ...
          this.custDeviceMeterSaveDeviceMeter.getData(this.custDeviceMeterSaveDeviceMeter.params).then(res => {
            ...
            this.$bus.emit('hideNewWatch')
            this.$bus.emit('updateElecWatchList', this.stid)
          })
      })
    }
  },
  destroyed () {
    ...
    this.$bus.off('updateElecWatchList', this.getStid)
