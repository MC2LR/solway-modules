<template>
    <div class="wrap">
        <div class="type">风电月发电量</div>
        <ve-histogram
            ref="myChart"
            :data="chartData"
            :after-config="afterConfig"
            height='100%'
            v-loading='queryWpCompanyMonthRangeElecTimeline.loading'
        ></ve-histogram>
    </div>
</template>
...
import { api＿queryWpCompanyMonthRangeElecTimeline } from '@newEnergyCloud/request/api'
export default {
    data () {
        return {
            queryWpCompanyMonthRangeElecTimeline: api＿queryWpCompanyMonthRangeElecTimeline({
                later: true
            }),
    ...
    created () {
        ...
        this.queryWpCompanyMonthRangeElecTimeline.getData({
                ...
                data.real_kwh_tb.forEach(v => {
                    arr3.push({'同期电量(度)': v})
                })
                data.finish_r_m.forEach(v => {
                    arr4.push({'完成率(%)': v})
                })
    ...
    methods: {
        afterConfig (options) {
            if (this.queryWpCompanyMonthRangeElecTimeline.res.body) {
                const { data } = this.queryWpCompanyMonthRangeElecTimeline.res.body
                ...
                this.options.series[4].data = data.real_kwh_tb
                this.options.series[5].data = data.real_kwh_tb
                this.options.series[6].data = data.real_kwh_tb
                this.options.series[7].data = data.real_kwh_tb
