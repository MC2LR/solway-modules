<template>
    <div class="wrap">
        ...
        <ve-histogram
            ...
            :events="chartEvents"
            height='100%'
            v-loading='queryCompanyYearElecDetailStationlineHis.loading'
        ></ve-histogram>
    </div>
</template>
...
import { api＿queryCompanyYearElecDetailStationlineHis } from '@newEnergyCloud/request/api'
export default {
    data () {
        return {
            chartEvents: {},
            queryCompanyYearElecDetailStationlineHis: api＿queryCompanyYearElecDetailStationlineHis({
                later: true
            }),
    ...
    created () {
        ...
        this.chartEvents = {
            click: function (e) {
                const { st_name: stName, stid } = self.queryCompanyYearElecDetailStationlineHis.res.body.data
        ...
        this.queryCompanyYearElecDetailStationlineHis.getData({
                ...
                // 是不是因为这个数据没刷新过才不显示呢，这里resize好像不管用
                // this.$refs.myChart.echartsResize()
            })
    },
    methods: {
        afterConfig (options) {
            if (this.queryCompanyYearElecDetailStationlineHis.res.body) {
                const { data } = this.queryCompanyYearElecDetailStationlineHis.res.body
                let end
