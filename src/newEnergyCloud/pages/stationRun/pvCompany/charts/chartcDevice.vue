<template>
    <div class="wrap">
        <ve-histogram
            ...
            height='100%'
            ref='chart'
            v-loading='queryPvMonthElecCompanyStationlistV201.loading'
        ></ve-histogram>
    </div>
</template>
...
import { api＿queryPvMonthElecCompanyStationlistV201 } from '@newEnergyCloud/request/api'
...
export default {
    ...
    data () {
        return {
            ...
            showChartIndex: 0,
            pageTheme: '',
            queryPvMonthElecCompanyStationlistV201: api＿queryPvMonthElecCompanyStationlistV201({
                later: true,
                res: {
                    body: {
                        data: [],
    ...
    methods: {
        resizeChart (i) {
            ...
            if (this.showChartIndex !== 2 || this.tabIndex !== 1) return
            setTimeout(() => {
                this.getChartData()
        ...
        getChartData () {
            this.queryPvMonthElecCompanyStationlistV201.getData({
                    date: this.showTime
                }).then(res => {
                    this.chartData.rows = []
