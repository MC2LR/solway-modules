<template>
    <div class="wrap">
        <ve-histogram
            ...
            height='100%'
            ref='chart'
            v-loading='(pvMonitorGetRtKWHPowerList.resNumber === 0) && pvMonitorGetRtKWHPowerList.loading'
        ></ve-histogram>
    </div>
</template>
...
import { api＿pvMonitorGetRtKWHPowerList } from '@newEnergyCloud/request/api'
...
export default {
    ...
    data () {
        return {
            ...
            showChartIndex: 0,
            pageTheme: '',
            pvMonitorGetRtKWHPowerList: api＿pvMonitorGetRtKWHPowerList({
                later: true,
                sameTimeOnce: true,
                res: {
                    body: {
    ...
    created () {
        ...
        this.$bus.on('tabsDay', this.getTabs)
        this.$bus.on('stationChange', this.stationChange)
        this.$bus.on('switch_theme', this.themeChange)
        this.timer = setInterval(() => {
            if (this.isToday) this.resizeChart(this.showChartIndex)
        }, 20000)
    },
    methods: {
        resizeChart (i) {
            this.showChartIndex = i
            if (this.showChartIndex !== 1 || this.tabIndex !== 1) return
            setTimeout(() => {
                this.getChartData()
        ...
        getTime (date) {
            ...
            this.isToday && this.resizeChart(this.showChartIndex)
        },
        themeChange (theme) {
            this.pageTheme = this.theme.$a
            this.resizeChart(this.showChartIndex)
        ...
        getChartData () {
            try {
                this.pvMonitorGetRtKWHPowerList.getData({
                        date: this.showTime
                    }).then(res => {
                        this.chartData.rows = []
                        if (!res.body || Object.keys(res.body).length === 0) return
                        res.body.forEach(v => {
                            this.chartData.rows.push({'电站': v.st_name, '本期电量': v.real_kwh, '同期电量': v.real_kwh_tb})
                        })
                        let end = 20 / res.body.length
    ...
    beforeDestroy () {
        ...
        this.$bus.off('tabsDay', this.getTabs)
