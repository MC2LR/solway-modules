<template>
    <div class="wrap">
        <ve-histogram
            ...
            height='100%'
            ref='chart'
            v-loading='queryPvAccumElecTimelistV201.loading'
        ></ve-histogram>
        <el-select
        v-model="legendIndex"
        class="legendChange"
...
import { api＿queryPvAccumElecTimelistV201 } from '@newEnergyCloud/request/api'
...
export default {
    ...
    data () {
        return {
            ...
            chartExtend: {},
            queryPvAccumElecTimelistV201: api＿queryPvAccumElecTimelistV201({
                later: true,
                res: {
                    body: {
                        data: [],
    ...
    methods: {
        ...
        changeLegend () {
            this.chartData.rows = []
            const { data } = this.queryPvAccumElecTimelistV201.res.body
            if (this.legendIndex === 1) {
                data.forEach(v => {
                    this.chartData.rows.push({'时间': v.dtime, '本期电量': v.real_kwh_chk, '完成率': v.finish_r})
                })
                ...
                data.forEach(v => {
                    this.chartData.rows.push({'时间': v.dtime, '本期电量': v.real_kwh_chk, '辐射量': v.rad_chk})
                })
                this.chartData.columns = ['时间', '本期电量', '辐射量']
                this.chartExtend = {
                    yAxis (v, i) {
                        v.forEach((vv, ii) => {
                            vv.axisLine = {
        ...
        getChartData () {
            this.queryPvAccumElecTimelistV201.getData({
                    date: this.showTime
                }).then(res => {
                    this.chartData.rows = []
