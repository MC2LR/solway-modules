...
import { api＿getStationRtOverviewComDay } from '@newEnergyCloud/request/api'
...
export default {
    ...
    data () {
        return {
            ...
            sectionShadow: 0,
            pvwpStationRtOverviewDay: api＿getStationRtOverviewComDay({
                later: true,
                sameTimeOnce: true,
                res: {
                    body: {
    ...
    methods: {
        ...
        getData () {
            if (this.$store.getters.station_data_type !== 'WP_COMPANY') return
            try {
