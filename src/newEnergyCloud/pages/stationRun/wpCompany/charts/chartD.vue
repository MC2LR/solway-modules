<template>
    <div class="wrap">
        <ve-histogram
            ...
            height='100%'
            ref='chart'
            v-loading='queryWpAccumElecTimelistV202.loading'
        ></ve-histogram>
        <el-select
        v-model="legendIndex"
        class="legendChange"
...
import { api＿queryWpAccumElecTimelistV202 } from '@newEnergyCloud/request/api'
...
export default {
    ...
    data () {
        return {
            ...
            chartExtend: {},
            queryWpAccumElecTimelistV202: api＿queryWpAccumElecTimelistV202({
                later: true,
                res: {
                    body: {
                        data: [],
    ...
    methods: {
        ...
        changeLegend () {
            this.chartData.rows = []
            const { data } = this.queryWpAccumElecTimelistV202.res.body
        ...
        getChartData () {
            this.queryWpAccumElecTimelistV202.getData({
                    date: this.showTime
                }).then(res => {
                    this.chartData.rows = []
