<template>
    <div class="wrap">
        <ve-histogram
            ...
            height='100%'
            ref='chart'
            v-loading='(wpMonitorDataGetPvRunAcpTimeList.resNumber === 0) && wpMonitorDataGetPvRunAcpTimeList.loading'
        ></ve-histogram>
    </div>
</template>
...
import { api＿wpMonitorDataGetPvRunAcpTimeList } from '@newEnergyCloud/request/api'
...
export default {
    ...
    data () {
        return {
            ...
            stime: 0,
            pageTheme: '',
            wpMonitorDataGetPvRunAcpTimeList: api＿wpMonitorDataGetPvRunAcpTimeList({
                later: true,
                sameTimeOnce: true
            }),
    ...
    methods: {
        ...
        getChartData () {
            this.stime = this.wpMonitorDataGetPvRunAcpTimeList.res.body
                    ? this.wpMonitorDataGetPvRunAcpTimeList.res.body.echarts_xaxisTime[this.wpMonitorDataGetPvRunAcpTimeList.res.body.echarts_xaxisTime.length - 1].split(':').join('')
                    : 0
            try {
                this.wpMonitorDataGetPvRunAcpTimeList.getData({
                    ...
                    stime: this.stime
                        ...
                        if (res.body.echarts_xaxisTime[0] === this.stationDayTodayTimeLineData.xdata[0]) return
                        const { xdata, line1, line2 } = this.stationDayTodayTimeLineData
                        const length = xdata.length

                        this.stationDayTodayTimeLineData.xdata = xdata.slice(0, length - 1).concat(res.body.echarts_xaxisTime)
                        this.stationDayTodayTimeLineData.line1 = line1.slice(0, length - 1).concat(res.body.echarts_power)
                        this.stationDayTodayTimeLineData.line2 = line2.slice(0, length - 1).concat(res.body.echarts_windSpeed)
                        let arr1 = []
                        let arr2 = []
                        let arr3 = []
                        this.chartData.rows = []
        ...
        setDataNull () {
            this.wpMonitorDataGetPvRunAcpTimeList.res.body = null
