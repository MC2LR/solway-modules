<template>
    <div class="wrap">
        <ve-histogram
            ...
            height='100%'
            ref='chart'
            v-loading='queryWpDayElecStationDevlistV201.loading'
        ></ve-histogram>
    </div>
</template>
...
import { api＿queryWpDayElecStationDevlistV201 } from '@newEnergyCloud/request/api'
...
export default {
    ...
    data () {
        return {
            ...
            showChartIndex: 0,
            pageTheme: '',
            queryWpDayElecStationDevlistV201: api＿queryWpDayElecStationDevlistV201({
                later: true,
                res: {
                    body: {
                        data: [],
    ...
    methods: {
        ...
        getChartData () {
            ...
            this.queryWpDayElecStationDevlistV201.getData({
                    date: this.showTime
                }).then(res => {
                    this.chartData.rows = []
                    ...
                    const { data } = res.body
                    data.forEach(v => {
                        this.chartData.rows.push({'设备': v.eq_name, '本期电量': v.real_kwh, '同期电量': v.real_kwh_tb})
                    })
            })
        },
        afterConfig (options) {
