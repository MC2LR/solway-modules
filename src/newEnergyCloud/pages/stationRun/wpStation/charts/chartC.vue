<template>
    <div class="wrap">
        <ve-histogram
            ...
            height='100%'
            ref='chart'
            v-loading='queryWpYearElecTimelistV201.loading'
        ></ve-histogram>
        <el-select
        v-model="legendIndex"
        class="legendChange"
...
import { api＿queryWpYearElecTimelistV201 } from '@newEnergyCloud/request/api'
...
export default {
    ...
    data () {
        return {
            ...
            chartExtend: {},
            queryWpYearElecTimelistV201: api＿queryWpYearElecTimelistV201({
                later: true,
                res: {
                    body: {
                        data: [],
    ...
    methods: {
        ...
        changeLegend () {
            this.chartData.rows = []
            const { data } = this.queryWpYearElecTimelistV201.res.body
            if (this.legendIndex === 1) {
                data.forEach(v => {
                    this.chartData.rows.push({'时间': v.dtime, '本期电量': v.real_kwh, '同期电量': v.real_kwh_tb, '完成率': v.finish_r})
                })
                ...
                data.forEach(v => {
                    this.chartData.rows.push({'时间': v.dtime, '本期电量': v.real_kwh, '同期电量': v.real_kwh_tb, '风速': v.ws_avg})
                })
                this.chartData.columns = ['时间', '本期电量', '同期电量', '风速']
                this.chartExtend = {
                    yAxis (v, i) {
                        v.forEach((vv, ii) => {
                            vv.axisLine = {
        ...
        getChartData () {
            ...
            this.queryWpYearElecTimelistV201.getData({
                    date: this.showTime
                }).then(res => {
                    this.chartData.rows = []
