<template>
    <div class="more-info">
        ...
        <div class="parms">
            <span>塔高：{{towerData.sh1}}米</span>
            <span>经度：{{towerData.longitude}}</span>
            <span>纬度：{{towerData.latitude}}</span>
        </div>
        <div class="hours">
            <el-row>
                <el-col :span="4">
...
export default {
    ...
    created () {
        this.$bus.on('devicePopupTowerMoreInfo', this.onData)
    },
    methods: {
        onData (data) {
            if (data?.b?.c?.d) {}
    ...
    beforeDestroy () {
        this.$bus.off('devicePopupTowerMoreInfo', this.onData)
    }
}
</script>
