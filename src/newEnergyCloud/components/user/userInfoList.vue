    ...
    <div class="user-info-list" @click.stop="">
        <ul class="dropdown-menu-list">
            <li v-for="role in rolelist" v-bind:key="role.i">
                 <a @click="changeRole(role.roleId)" :title="role.roleDisplayName">
...
<script>
import {api＿logout, logout} from '@comm/request/api'
...
export default {
    ...
    methods: {
        async logOut () {
            await api＿logout({later: true}).getData()
            await logout({later: true}).getData()
